...
#
# api.json and symbols.txt are committed so the build does not depend on
# running Python, but a committed generated file can silently go stale.
# This regenerates everything into a scratch copy of the tree and diffs;
# any drift fails.
...
cp -R "$repo/scripts/gen_api.py" "$repo/scripts/reference_exports.txt" \
      "$work/scripts/"
...
status=0
for file in scripts/api.json scripts/symbols.txt; do
